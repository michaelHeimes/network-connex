 ...
 */
function network_connex_setup() {
	/*
		* Make theme available for translation.
		* Translations can be filed in the /languages/ directory.
		* If you're building a theme based on 3D Lacrosse, use a find and replace
		* to change 'network-connex' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'network-connex', get_template_directory() . '/languages' );
	...
	// register_nav_menus(
	// 	array(
	// 		'menu-1' => esc_html__( 'Primary', 'network-connex' ),
	// 	)
	// );
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'network_connex_custom_background_args',
			array(
...
add_action( 'after_setup_theme', 'network_connex_setup' );
 ...
 */
function network_connex_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'network_connex_content_width', 640 );
}
add_action( 'after_setup_theme', 'network_connex_content_width', 0 );
 ...
 */
function network_connex_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'network-connex' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'network-connex' ),
	...
	register_sidebar(array(
		'id' => 'offcanvas',
		'name' => __('Offcanvas', 'network-connex'),
		'description' => __('The offcanvas sidebar.', 'network-connex'),
...
add_action( 'widgets_init', 'network_connex_widgets_init' );


/**
 * Enqueue scripts and styles.
 */
function network_connex_scripts() {
	wp_enqueue_style( 'network-connex-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'network-connex-style', 'rtl', 'replace' );
	
	wp_enqueue_style( 'network-connex-style-min', get_template_directory_uri() . '/assets/styles/style.min.css', array(), _S_VERSION );
...
add_action( 'wp_enqueue_scripts', 'network_connex_scripts' );
